...
foreach ($i in 0, 1, 2) {
    ...
    if ($process.ExitCode -eq 0) {
        ...
        Get-ChildItem $env:Temp\dd_bootstrapper_*.log |Sort-Object CreationTime -Descending |Select-Object -First 1 |ForEach-Object {
            Write-Host "${_}:"
            Get-Content "$_"
            Write-Host
        }

        $clientLogs = Get-ChildItem $env:Temp\dd_client_*.log |Sort-Object CreationTime -Descending |Select-Object -First 1 |ForEach-Object {
            Write-Host "${_}:"
            Get-Content "$_"
            Write-Host
        }

        $setupLogs = Get-ChildItem $env:Temp\dd_setup_*.log |Sort-Object CreationTime -Descending |Select-Object -First 1 |ForEach-Object {
            Write-Host "${_}:"
            Get-Content "$_"
            Write-Host
        }
